...
describe NuntiumController do
  ...
  describe "POST 'receive'" do
    ...
    it "should confirm candidate" do
      Candidate.expects(:find_last_for_sms_number).with(@candidate.volunteer.sms_channels.first.address).returns(@candidate)
    ...
    ["Yes I Can!", "I think I will be available, yes"].each do |answer|
      it "should allow more flexible responses for confirmation" do
        Candidate.expects(:find_last_for_sms_number).with(@number).returns(@candidate)
        @candidate.expects(:answered_from_sms!).with("yes")
        post 'receive', :from => @number.with_protocol, :body => answer
      end
    end
